...
int i = 0;
//printf(">>%s<<   ",fn_cpy);
   for (tok = strtok_r (fn_cpy, " ", &save_ptr); tok != NULL; tok = strtok_r (NULL, " ", &save_ptr))
     { arg_MASS[i++] = tok; 
  // printf(">>%s<<   ",arg_MASS[i-1]);
}
 for(int j = i-1; j>=0; j--){
*esp= *esp - (strlen(arg_MASS[j])+1);
strlcpy(*esp, arg_MASS[j], strlen(arg_MASS[j])+1);
ptr_MASS[j] = *esp;
 }
